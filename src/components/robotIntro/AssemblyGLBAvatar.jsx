...
function AssemblyGLBAvatarLite({ progress }) {
  const materials = useMemo(() => {
    const base = createRobotMaterials();
    const suit = base.gunmetal.clone();
    suit.color.set('#141920');
    const suitCore = base.gunmetal.clone();
    suitCore.color.set('#0f1318');
    return {
      suit,
      suitCore,
      suitPlate: base.carbon.clone(),
      suitSole: base.carbon.clone(),
      suitSeam: base.gunmetal.clone(),
      suitJoint: base.silver.clone(),
    };
  }, []);

  return (
    <group position={[0, INTRO_WAIST_Y, 0]}>
      <AssemblySuitLegs progress={progress} hipY={0} materials={materials} />
    </group>
  );
}
...
export default function AssemblyGLBAvatar({ progress, lite = false }) {
  if (lite) return <AssemblyGLBAvatarLite progress={progress} />;
